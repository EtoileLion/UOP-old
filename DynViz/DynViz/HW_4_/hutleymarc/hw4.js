...
var updateBarplot = function() {
	var selected = d3.select("#selectData").property("value");
	var range = d3.extent(data_dict.locale_data,function(elem) { return elem[selected]; });
	var yScale = d3.scaleLinear().domain(range)
                 .range([height - padding, padding]);
	var yAxis = d3.axisLeft(yScale);
    var rects = d3.select("svg").selectAll("rect").data(data_dict.locale_data);
	rects.merge(rects)
	.transition(1500)
    .attr('y', function(elem) {
		if(isNaN(elem[selected]) || elem[selected] === null)  { elem[selected] = range[0]; }
       return yScale(elem[selected]);
    })
	.attr('height', function(elem) {
		if(isNaN(elem[selected]) || elem[selected] === null)  { elem[selected] = range[0]; }		
       return height - yScale(elem[selected]);
    });	
	d3.select(".axis").call(yAxis);
	//Meh. Text doesnt transition. Pity.
	d3.select(".title").transition(1500).text(selected);
}
/**
 * @typedef LocaleCrime
 * @type Object
...
//I need this data elsewhere. Global variables are NOT the devil.
var data_dict = {}
var width = 800;
var height = 500;
var padding = 30;

//Make the select box do something.
d3.select("#selectData").on("change",updateBarplot);
...
d3.json("../data/mini_Chicago_crime_records.json",
  ...
  if (error) {
    ...
    data_dict = orgData(data);
    performBarplot(data_dict.locale_data, data_dict.nested_crime);
	//Meh, not the cleanest way of doing it, but it works. I could hardcode them, but... no.
	Object.keys(data_dict['locale_data'][0]).forEach(function (elem) {
		if(elem == "CA Name") { return; }
		var option = document.createElement("option");
		option.text = elem;
		document.getElementById("selectData").add(option);
	});
  }
});
