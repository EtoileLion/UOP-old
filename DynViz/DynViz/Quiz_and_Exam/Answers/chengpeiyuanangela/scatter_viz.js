...
var parm = {
  category: {
    tag: "#category", name: "category", text: "NY categories"
  },
  event_name: {
    tag: "#event_name", name: "event_name", text: "event_name"
  },
  free: {
    tag: "#free", name: "Area Prop Age>16 free",
    text: "free"
  },
  kid_friendly: {
    tag: "#kid_friendly", name: "kid friendly",
    text: "kid_friendly"
  },
  venue_name: {
    tag: "#venue_name", name: "venue_name", text: "venue_name "
  },
  web_description: {
    tag: "#web_description", name: "web_description", text: "web_description"
  }
}
var dim = {
...
var clicked = {
  x: parm.event_name,
  y: parm.kid_friendly,
  size: parm.venue_name,
  color: parm.event_name,
};
*/
...
// for the web_description, which range from 0 - 8.
...
var showDetails = function(that) {
  ...
  d3.select(parm.category.tag).text(datum[parm.category.name]);
  d3.select(parm.event_name.tag)
    .text(av.roundPretty(datum[parm.event_name.name], 2).toLocaleString());
  d3.select(parm.free.tag)
    .text(+(datum[parm.free.name] * 100).toFixed(1));
  d3.select(parm.kid_friendly.tag)
    .text(+(datum[parm.kid_friendly.name] * 100).toFixed(1));
  d3.select(parm.venue_name.tag).text(datum[parm.venue_name.name]);
  d3.select(parm.web_description.tag).text(datum[parm.web_description.name]);
};
...
var updatePoints = function(data, points, clicked) {
  ...
  svg.select("#title1")
     .transition("updating")
     .duration(dim.transition)
     .text(parm.category.text + "; " + clicked.y.text);
  svg.select("#title2")
     .transition("updating")
     .duration(dim.transition)
...
d3.json("./data/nytimes_event_listings.json",
  ...
  if (error) {
    ...
    points.on("mouseover", function(elem) {
          ...
          .append("title")
          .text(function(elem) {
             if (elem[parm.web_description.name] === 0) {
               return elem[parm.category.name] + ":  No web_description";
             } else if (elem[parm.web_description.name] === 1) {
               return elem[parm.category.name] + ":  1 Arrest";
             } else {
               return elem[parm.category.name] + ":  "
                 + elem[parm.web_description.name] + " web_description";
             }
           });
