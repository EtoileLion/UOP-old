...
var cleanData = function(incoming_data) {
      ...
      incoming_data = _.map(incoming_data, function(elem) {
        return _.pick(elem, 'subcategory', 'category', 'event_name', 'event_url', 'free',
      ...
      incoming_data.forEach(function(elem) {
        if (elem.subcategory === undefined | elem.subcategory === null) {
          elem.subcategory = 'None/Not Specified';
        }
      });
...
var main = function(error, raw_data) {
  ...
  yScale.domain([0.1, d3.max(data, function(elem) {
  ...
  labels.attr("transform", function(elem) {
           return "translate(" + (xScale.bandwidth() + xScale(elem.key) - 5)
 ...
 bars.on("click", function() {
	var show_details = d3.select("#tooltip").classed("hidden");
          ...
          .append("title")
          .text("Example event: " + data[0].values[0].event_name +
              data[0].values[0].web_description);
         //  });
};
