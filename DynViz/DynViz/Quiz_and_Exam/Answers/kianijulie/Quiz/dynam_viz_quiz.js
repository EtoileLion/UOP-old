...
var cleanData = function(incoming_data) {
      ...
      incoming_data = _.map(incoming_data, function(elem) {
        return _.pick(elem, 'subcategory', 'event_name', 'event_detail_url', 'free',
      ...
      incoming_data.forEach(function(elem) {
        if (elem.subcategory === undefined | elem.subcategory === null) {
          elem.subcategory = 'No Subcategory Listed';
        }
      });
      ...
      // Calculate the number of events associated with each subcategory.
...
var main = function(error, raw_data) {
  ...
  bars.attr("width", xScale.bandwidth())
      ...
      .on("click", function() {
       ...
       .append("event_name")
       .text(function(elem) {
          if (elem.event_detail_url === undefined | elem.event_detail_url === null) {
            return "Event URL: No event link provided";
          } else {
            return "Event URL: "
              + elem.event_detail_url;
          }
      });
